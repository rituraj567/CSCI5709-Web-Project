...
function Transaction() {
  ...
  return (
    <div>
      {/* <p>{transactionData.map((transaction)=>(
        <div >{transaction.amount}
          {transaction.type}
        </div> */}
        <Header/>
        <Typography variant="h4" textAlign="center" sx={{py:10, fontWeight:"bold"}}>My Transactions</Typography>
         ...
         <Table sx={{ minWidth: 650 }} aria-label="simple table">
           <TableHead>
             <TableRow>
               ...
               <TableCell align="right" sx={{fontWeight:"bold"}}>Source</TableCell>
             </TableRow>
           </TableHead>
           <TableBody>
       ...
       <Button variant='contained' sx={{marginLeft:"45%", mt:3}} onClick={() => navigate("/wallet")}>Go Back</Button>
      {/* ))}</p> */}
    </div>
  )
}

...
const { UpdatePassword, getUserFirstandLastName, updateUserFirstandLastName, getUserAddress, updateAddress } = require("./accountcontroller");
...
router.post('/updatepassword', authenticateUser,  async (req, res) => {
  ...
  router.post("/updateuserfirstandlastName",authenticateUser,async(req,res)=>{
    try {
        const userId=req.user.id;
        const data=req.body;  
        console.log("userId"+userId);

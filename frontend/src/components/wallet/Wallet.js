...
function Wallet() {
  ...
  const addMoneyHandler= async()=>{
    ...
    axios
      .put(
        process.env.REACT_APP_BACKEND_SERVER +
          "/wallet/updateWallet",updatedBalance,
        {
          headers: headers,
        }
      )
  ...
  return (
    <div >
      ...
      <FormControl fullWidth sx={{ m: 1 }}>
        ...
        <Button variant='contained' sx={{width:"fit-content", mt:3}} onClick={addMoneyHandler}>Add amount</Button>
        <Button variant='text' sx={{width:"fit-content", mt:3, ml:3}} onClick={() => navigate("/transaction")}>View Transactions</Button>
      </div>
      </div>
    </div>
  )
